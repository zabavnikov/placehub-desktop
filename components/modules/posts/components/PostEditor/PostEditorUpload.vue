<template>
  <div
      class="cursor-pointer border border-dashed border-2 p-4 border-gray-200 flex items-center justify-center rounded text-gray-300 hover:text-indigo-300"
      :class="{'aspect-video': !schemaNotEmpty}"
    ...
    <div class="flex flex-col items-center justify-center">
      ...
      <p class="mt-4">Нажмите, чтобы добавить изображения</p>
    </div>
    <Upload
        :fields="['id', 'url']"
        ...
        model-type="posts"
        ref="upload"
        @update:modelValue="emit('addBlock', $event)"
    />
  </div>
</template>
